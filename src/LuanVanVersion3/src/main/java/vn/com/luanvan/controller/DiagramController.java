...
public class DiagramController {
	...
	public @ResponseBody String addUsecase (@RequestParam(value="ucArray") List<String> ucArray,
		...
		for (int i = 0; i < ucArray.size(); i++) {
			...
			if (ucAttr.length > 5) {
				if (!nhomucDao.hasNhomuc(ucAttr[5], project.getProjectid())) {
					...
					uc.setNhomuc(nhomucDao.getNhomucByName(ucAttr[5], project.getProjectid()));
				}
			}else{
				if (!nhomucDao.hasNhomuc("Không có nhóm", project.getProjectid())) {
					Nhomuc nhomuc = new Nhomuc();
					nhomuc.setProject(project);
					nhomuc.setTennhom("Không có nhóm");
					nhomucDao.add(nhomuc);
					uc.setNhomuc(nhomuc);
				}else{
					uc.setNhomuc(nhomucDao.getNhomucByName("Không có nhóm", project.getProjectid()));
				}
			}
			usecaseDao.add(uc);
		}
